...
const fetchFile = async (url) => {
  try {
    ...
    if (url.startsWith("file://") || url.includes(":\\") || url.includes("/")) {
      ...
      // 使用Tauri后端API读取文件
      const fileBytes = await invoke("read_file_bytes", { filePath });
      const blob = new Blob([new Uint8Array(fileBytes)]);
      return new File([blob], filePath.split(/[\\/]/).pop() || "unknown");
    } else {
      // 对于网络URL，继续使用fetch
    ...
    throw new ResponseError(`Failed to load file: ${error.message}`, {
      cause: error,
    });
  }
};
